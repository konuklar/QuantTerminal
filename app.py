...
def generate_demo_data(tickers, start_date, end_date):
    ...
    try:
        ...
        # Default to 1 year of data if parsing fails
...
def load_global_prices_cached(tickers: List[str], start_date, end_date) -> pd.DataFrame:
    """Load global asset prices with enhanced caching"""
    
    # Ensure dates are strings for cache key generation
    ...
    cache_key = f"prices_{hashlib.md5((str(sorted(tickers)) + start_date_str + end_date_str).encode()).hexdigest()}"
    ...
    data = load_global_prices_enhanced(tickers, start_date_str, end_date_str)
...
def main():
    ...
    try:
        # Convert dates to string format for the data loading function
        ...
        with progress_container:
            ...
            with st.spinner(f"Downloading market data for {len(all_tickers_to_load)} assets..."):
                ...
                prices = load_global_prices_cached(all_tickers_to_load, start_date_str, end_date_str)
                ...
                if (prices.empty or len(prices.columns) < 2) and st.session_state.use_demo_data:
                    ...
                    prices = generate_demo_data(all_tickers_to_load, start_date_str, end_date_str)
